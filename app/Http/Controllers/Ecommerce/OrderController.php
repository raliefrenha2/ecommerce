...
class OrderController extends Controller
{
	...
	private function sendMessage($order_id, $reason)
	{
		...
		if ($chat['ok']) {
			...
			$text = 'Hai Prima Store, OrderID ' . $order_id . ' Melakukan Permintaan Refund Dengan Alasan "'. $reason .'", Segera Dicek Ya!';
			//DAN KIRIM REQUEST KE TELEGRAM UNTUK MENGIRIMKAN PESAN
			return $this->getTelegram('https://api.telegram.org/bot'. $key .'/sendMessage', '?chat_id=' . $chat_id . '&text=' . $text);
		}
	}
}
